...
# --- NEW: EdgeCrop Transform using OpenCV ---/
# using Gaussian Blur
# class EdgeCrop:
#     """
...
#         np_img = np.array(img.convert('L')) # Convert to grayscale
#         h, w = np_img.shape

#         # 1. Pre-process: Blur the image to reduce noise
...
#         # 3. Find the top and bottom boundaries of the tissue
...
#         else:
#             # Fallback for images with no clear edges
#             top_boundary = h // 4
#             bottom_boundary = h - (h // 4)

#         # 4. Apply padding and ensure we don't go out of bounds
#         top_crop = max(0, top_boundary - self.padding)
#         bottom_crop = min(h, bottom_boundary + self.padding)

#         # 5. Crop the original color PIL image
...
class EdgeCrop:
    """
    A transform to find the main tissue band using Canny edge detection and crop it.
    This version uses Median Blur for better edge preservation.
    """
    def __init__(self, padding=20, blur_kernel_size=9, canny_threshold1=50, canny_threshold2=190):
        # These are hyperparameters you can tune
        self.padding = padding
        # Note: medianBlur takes a single odd integer for kernel size
    ...
    def __call__(self, img):
        ...
        # 1. Pre-process: Blur the image using Median Blur to reduce noise while preserving edges
...
batch_size = 32
learning_rate = 1e-5
save_path = "resnet50_edgecrop_quality.pt" # New save path
early_stopping_patience = 3

# Integrate Edge Cropping into transforms
...
best_val_acc = 0.0
epochs_no_improve = 0
print(f"\nTraining with EdgeCrop started on device: {device}\n")
